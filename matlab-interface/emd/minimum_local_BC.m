...
function [Index,Value]=minimum_local_BC(Matrix)
...
for i=2:m+1
    for j=2:n+1
        smaller=true(1);
        if     Matrix(i,j)>=Matrix(i-1,j-1),  smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i-1,j),    smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i-1,j+1),  smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i,j-1),    smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i,j+1),    smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i+1,j-1),  smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i+1,j),    smaller=false(1);
        elseif Matrix(i,j)>=Matrix(i+1,j+1),  smaller=false(1);
        end
        if smaller==true(1) Index=[Index;i-1 j-1];Value=[Value;Matrix(i,j)];end
    end
end
